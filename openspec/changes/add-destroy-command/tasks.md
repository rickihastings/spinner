...
## 1.0 Destroy command with state cleanup, multi-instance support, and spin output update
...
- [ ] 1.2 Update `cmd/spin.go` management instructions to replace backend-specific "To remove:" lines with
  `spinner destroy <instance-name>` for both Docker and GCP output blocks
- [ ] 1.3 Create `cmd/destroy_test.go` with unit tests using mock provider: destroy single running instance,
...
- [ ] 1.4 Verify build succeeds and all tests pass

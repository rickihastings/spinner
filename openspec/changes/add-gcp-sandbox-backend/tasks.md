...
- [x] 3.8 Add `--bake-script` flag to setup and spin commands — reads user script file, passes contents to bake template as `BakeScript` variable
- [x] 3.9 Update `templates/scripts/gcp_bake.sh` — add `{{.BakeScript}}` template block after core tooling, before shutdown
- [x] 3.10 Update `internal/gcp/startup.go` — accept `BakeScript` in template data, read file contents from options
- [x] 3.11 Add tests for `--bake-script` flag validation and template rendering with custom script
- [x] 3.12 Verify build and tests pass

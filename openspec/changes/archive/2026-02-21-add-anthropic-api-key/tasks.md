...
- [x] 1.1 Update `internal/secret/resolver.go`:
...
- [x] 1.2 Update `cmd/spin.go`:
  - Add `ANTHROPIC_API_KEY` to the `reserved` map.

- [x] 1.3 Update `internal/secret/resolver_test.go`:
...
- [x] 1.4 No changes to `tests/testutil/secret.go`:
...
- [x] 1.6 Verify: `go build ./...` passes; `go test ./...` passes.
- [x] 1.7 Commit with message: `feat(secret): support ANTHROPIC_API_KEY as claude auth alternative`.

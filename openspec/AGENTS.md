...
Each agent invocation implements ONE complete feature (X.0). Do NOT scatter changes across the codebase or defer tests.
The vertical slice must be fully implemented, tested, and committed—then the agent's job is done.
...
- On completion, halt, the job is completed.

**IMPORTANT**

IF there are no remaining vertical slices beyond this, the feature is regarded as complete. Output the signal
`~~ FEATURE_COMPLETED ~~`.
